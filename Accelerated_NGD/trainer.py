...
class Trainer:
    ...
    def plot_grads(self, grads=None, label='', marker=',', markevery=None):
        n_plot = 400
        skip = len(grads) // n_plot
        plt.plot(np.arange(0, len(grads)-1), grads[1:], label=label, marker=marker, markevery=markevery)
